...
# 10 независимых seeds. Обоснование (пункт M1/M2): при 3 seeds минимальный
# детектируемый эффект ~12.6 п.п. и эквивалентность (TOST) не доказывается.
# При 10 seeds n на конфигурацию вырастает с ~246 до ~820, MDE падает до
# ~7 п.п. — этого достаточно, чтобы СТРОГО доказать отсутствие эффекта
# квантизации, а не просто "не найти" его.
...
def main():
    ...
    for cfg in MODELS_TO_TEST:
        ...
        # M4: капризные к формату архитектуры получают strict_format,
        # чтобы измерять их уязвимость, а не harness-mismatch
        needs_strict = cfg.get("arch") in ("phi", "gemma")
        agent = SimulatedAgent(client, name, strict_format=needs_strict)
